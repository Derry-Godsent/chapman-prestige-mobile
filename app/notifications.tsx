...
import { BodyText, PrimaryButton, StatusPill, palette } from "@/components/chapman-ui";
...
const KIND_STYLE: Record<CustomerNotification["kind"], { icon: keyof typeof Ionicons.glyphMap; color: string; label: string }> = {
  ...
  booking: { icon: "cube-outline", color: palette.blue, label: "BOOKING" },
  ...
  return (
    <AppScreen>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {notice ? <View style={styles.notice}><Ionicons name="information-circle-outline" size={18} color={palette.blue} /><Text style={styles.noticeText}>{notice}</Text></View> : null}

        <View style={styles.sectionHead}>
          <Text style={styles.sectionLabel}>YOUR SERVICE UPDATES</Text>
          <TouchableOpacity onPress={() => void reload()} style={styles.refresh}><Ionicons name="refresh" size={15} color={palette.blue} /></TouchableOpacity>
        </View>

        {loading ? (
          <View style={styles.loading}><ActivityIndicator color={palette.blue} /><Text style={styles.loadingText}>Checking your account</Text></View>
        ) : !signedIn ? (
          <View style={styles.emptyCard}>
            <Ionicons name="phone-portrait-outline" size={24} color={palette.blue} />
          ...
          <View style={styles.emptyCard}>
            <Ionicons name="notifications-off-outline" size={24} color={palette.blue} />
        ...
        <TouchableOpacity onPress={() => router.push("/services" as never)} style={styles.explore}>
          ...
          <Ionicons name="arrow-forward" size={18} color={palette.blue} />
        </TouchableOpacity>
      </ScrollView>
    </AppScreen>
  );
...
const styles = StyleSheet.create({
  ...
  notice: { padding: 12, borderRadius: 14, backgroundColor: "#EEF3FF", flexDirection: "row", gap: 8, alignItems: "center" },
  ...
  sectionLabel: { color: "#5871B5", fontFamily: "Inter_700Bold", fontSize: 9, letterSpacing: 1.15 },
  refresh: { width: 32, height: 32, borderRadius: 11, backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: palette.border, alignItems: "center", justifyContent: "center" },
  loading: { padding: 34, alignItems: "center", gap: 9 },
  loadingText: { color: palette.muted, fontFamily: "Inter_500Medium", fontSize: 11 },
  emptyCard: { padding: 22, borderRadius: 21, backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: palette.border, alignItems: "center", gap: 9 },
  ...
  updateCard: { padding: 15, borderRadius: 19, borderWidth: 1, borderColor: palette.border, backgroundColor: "#FFFFFF", flexDirection: "row", alignItems: "flex-start", gap: 11 },
  ...
  exploreText: { color: palette.blue, fontFamily: "Inter_700Bold", fontSize: 13 },
});
