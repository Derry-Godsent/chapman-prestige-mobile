...
export default function BookingDetailScreen() {
  ...
  const serviceMeta = isQuote ? `${quote?.propertyType} · ${quote?.preference}` : liveRequest ? `${readableDate(liveRequest.requested_for)} · ${liveRequest.pickup_window ?? "time to be confirmed"}` : booking?.scheduledFor;
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
              ...
              {appointment === "awaiting-customer" ? (
                <>
                  ...
                  <View style={styles.appointmentActions}>
                    <TouchableOpacity onPress={() => respondToAppointment(id, "rejected")} style={styles.rejectButton}><Text style={styles.rejectButtonText}>Reject date</Text></TouchableOpacity>
                    <TouchableOpacity onPress={() => respondToAppointment(id, "accepted")} style={styles.acceptButton}><Text style={styles.acceptButtonText}>Accept date</Text><Ionicons name="checkmark" size={17} color="#FFFFFF" /></TouchableOpacity>
                  </View>
                </>
              ) : (
            ...
            <View style={[styles.appointmentCard, isDeclined && styles.declinedCard]}>
              <View style={styles.appointmentHeader}>
                ...
                <View style={styles.appointmentCopy}>
                  ...
                  <Text style={[styles.appointmentDate, isDeclined && styles.declinedAppointmentDate]}>{isDeclined ? "No service date was confirmed" : readableDate(liveRequest?.requested_for)}</Text>
                </View>
              </View>
              {liveAwaitingResponse ? (
                <>
                  <Text style={styles.appointmentText}>Accept this date to approve your Laundry service, or reject it to close this request.</Text>
                  <View style={styles.appointmentActions}>
          ...
          <View style={styles.progressSection}>
            <View style={styles.progressHeader}>
              ...
              <TouchableOpacity onPress={() => void loadLiveRequest()} disabled={!shouldLoadLiveRequest || liveLoading} style={styles.refreshButton}>
          ...
          <View style={[styles.specialistCard, isDeclined && styles.declinedCard]}>
            ...
            <View style={styles.specialistCopy}>
              ...
              <Text style={styles.specialistText}>{booking?.specialistName ? "Your assigned specialist is responsible for this service update." : isDeclined ? "Message Chapman for help with another Laundry date or a different service option." : requestInReview ? "A coordinator is checking your request before sharing a date for your approval." : "We show the person, arrival update, and service details here after confirmation."}</Text>
            </View>
          </View>
        </ScrollView>
