...
import { ChapmanBonusCard } from "@/components/chapman-bonus-card";
...
import { formatBirthday, isBirthdayToday } from "@/lib/customer-auth";
import { laundryStanding, serviceStanding } from "@/lib/loyalty";
...
export default function ProfileScreen() {
  ...
  // The switch is drawn from the customer's choice, but the moment it is tapped
  // it shows what was tapped, so it can never feel stuck while the app repaints.
  const [wantedScheme, setWantedScheme] = useState<"light" | "dark" | null>(null);
  const darkOn = wantedScheme ? wantedScheme === "dark" : colorScheme === "dark";
  useEffect(() => {
    if (wantedScheme && colorScheme === wantedScheme) setWantedScheme(null);
  }, [colorScheme, wantedScheme]);
  const { activity, loading: summaryLoading } = useCustomerSummary();
  const birthday = account ? formatBirthday(account.birth_day, account.birth_month) : null;
  ...
  return <AppScreen><ScrollView style={styles.scroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    <View style={styles.header}><Text style={styles.eyebrow}>YOUR ACCOUNT</Text><View style={styles.headerButtons}><TouchableOpacity onPress={() => router.push("/team" as never)} style={styles.settingsButton} accessibilityLabel="The Chapman team, and where to send your ideas"><Ionicons name="people-outline" size={21} color={palette.ink} /></TouchableOpacity><TouchableOpacity onPress={() => router.push("/settings" as never)} style={styles.settingsButton} accessibilityLabel="App settings"><Ionicons name="settings-outline" size={21} color={palette.ink} /></TouchableOpacity></View></View>
    <TouchableOpacity onPress={() => router.push(account ? "/account" as never : "/auth/phone" as never)} style={styles.profileBlock}>
      <View style={styles.avatar}><Ionicons name={avatarIcon} size={27} color={palette.accent} /></View><View style={styles.profileCopy}><DisplayText style={styles.name}>{accountName}</DisplayText><Text style={styles.phone}>{account === undefined ? "Checking account" : accountDetail}</Text>{birthday ? <View style={styles.birthdayRow}><Ionicons name={birthdayToday ? "gift" : "calendar-outline"} size={12} color={birthdayToday ? palette.orange : palette.muted} /><Text style={[styles.birthdayText, birthdayToday && styles.birthdayTextToday]}>{birthdayToday ? `Happy birthday today, ${birthday} is your day` : `Birthday ${birthday}`}</Text></View> : null}</View><Ionicons name="chevron-forward" size={21} color={palette.muted} />
    </TouchableOpacity>
    {checking ? <View style={styles.checkingCard}><ActivityIndicator color={palette.accent} /><Text style={styles.checkingText}>Checking your account</Text></View> : !account ? <View style={styles.signInCard}><View style={styles.signInCopy}><Text style={styles.signInTitle}>Keep every booking in one place.</Text><Text style={styles.signInText}>Sign in with your phone to receive real dates, tracking, and service updates.</Text></View><PrimaryButton label="Sign in with phone" icon="phone-portrait-outline" onPress={() => router.push("/auth/phone" as never)} style={styles.signInButton} /></View> : <ChapmanBonusCard
      laundryTrack={laundryTrack}
    ...
    <View style={styles.settingsCard}>
      ...
      <View style={styles.settingBlock}><View style={styles.settingIcon}><Ionicons name="moon-outline" size={19} color={palette.accent} /></View><View style={styles.settingCopy}><Text style={styles.settingLabel}>Dark mode</Text><Text style={styles.settingDetail}>Use a darker app appearance in low light.</Text></View><Switch value={darkOn} onValueChange={(value) => { setWantedScheme(value ? "dark" : "light"); setColorScheme(value ? "dark" : "light"); }} trackColor={{ false: "#D7D1C7", true: "#81C5A7" }} thumbColor={palette.blue} /></View>
