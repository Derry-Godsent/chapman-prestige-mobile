...
function requestLabel(request: MobileLaundryRequest) {
  const { status } = { status: request.request_status };
...
export default function BookingsScreen() {
  ...
  useEffect(() => {
    ...
    if (!client) return;
    const channel = client.channel("customer-booking-list")
      .on("postgres_changes", { event: "*", schema: "public", table: "mobile_requests" }, () => { void loadLiveRequests(); })
      .subscribe();
  ...
  return (
    <AppScreen>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {liveRequests.map((request) => <TouchableOpacity key={request.id} onPress={() => router.push(`/booking/${request.id}` as never)} style={styles.bookingCard} activeOpacity={0.82}><View style={styles.cardTop}><IconOrb icon={request.request_status === "declined" ? "close-outline" : "shirt-outline"} color={request.request_status === "declined" ? palette.error : palette.green} /><View style={styles.cardCopy}><Text style={styles.bookingTitle}>Laundry & Garment Care</Text><Text style={styles.bookingMeta}>{request.request_status === "declined" ? request.customer_response === "rejected" ? "You rejected the proposed date. This request is closed." : "Chapman declined this request" : `${requestDate(request)} · ${request.pickup_window ?? "time to be confirmed"}`}</Text></View><Ionicons name="chevron-forward" size={20} color="#7A7E8D" /></View><View style={styles.cardFoot}><StatusPill label={requestLabel(request)} tone={request.request_status === "needs_customer_confirmation" ? "orange" : request.request_status === "confirmed" ? "green" : request.request_status === "declined" ? "red" : "blue"} /><Text style={styles.price}>{request.estimated_total === null ? "Estimate pending" : `₵${Number(request.estimated_total).toFixed(0)}`}</Text></View></TouchableOpacity>)}
