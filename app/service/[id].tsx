...
const serviceDetails: Record<string, { summary: string; steps: string[]; guidance: string[]; promise: string }> = {
  ...
  fumigation: { summary: "Tell us the pest problem and the kind of building. Our team will recommend the right treatment.", steps: ["Tell us the pest problem", "Choose a suitable day", "Receive a clear treatment quote"], guidance: ["Cockroach care from ₵250", "Bedbug care from ₵500", "Minimum service ₵300"], promise: "You receive clear preparation guidance and a treatment plan before the visit." },
  detailing: { summary: "Choose the care your car needs. We confirm the finish, the time, and the price before service starts.", steps: ["Tell us your car type", "Choose basic or full detail", "Confirm a service time"], guidance: ["Sedan wash ₵45", "SUV wash 60", "Full detail from ₵180"], promise: "We record the chosen package before work begins, so you know what to expect." },
  fabric: { summary: "Choose the furniture or carpet that needs care. We check the fabric and give you the right price.", steps: ["Tell us what needs cleaning", "Choose extra stain or odour care", "Confirm the final service price"], guidance: ["Sofa care from ₵60", "Sofa service minimum 150", "Carpet service minimum ₵200"], promise: "Use the measurement guide for a clearer carpet estimate. The final scope is always approved by you." },
  ...
  const [loadingRoutines, setLoadingRoutines] = useState(true);
    ...
    const loadRoutines = async () => {
      ...
      try {
        ...
        const localCadences = routines.filter((r) => r.serviceId === service.id).map((r) => r.cadence);
        setSavedCadences(localCadences);
      } finally {
          ...
          {servicePhoto ? (
            <View style={styles.photoFrame}>
              ...
              <View style={styles.photoLabel}>
                <Text style={styles.photoLabelText}>CHAPMAN PROFESSIONAL CARE</Text>
              </View>
            </View>
          ) : (
          ...
          <View style={styles.priceSection}>
            ...
            {detail.guidance.map((item) => (
              <View key={item} style={styles.priceLine}>
                ...
                <Text style={styles.priceLineText}>{item}</Text>
              </View>
            ))}
          </View>
            ...
            <View style={styles.routineCard}>
              ...
              {loadingRoutines ? (
                ...
                <View style={styles.routineOptions}>
                  {routineOptions.map((cadence) => {
                    const isSaved = savedCadences.includes(cadence);
                    return (
                      <TouchableOpacity 
                        key={cadence} 
                        activeOpacity={0.78} 
                        onPress={() => handleRoutineToggle(cadence)} 
                        style={[styles.routineChoice, isSaved && styles.routineChoiceSaved]}
                      >
                        {isSaved && <Ionicons name="checkmark" size={16} color="#FFFFFF" style={styles.checkIcon} />}
                        <Text style={[styles.routineChoiceText, isSaved && styles.routineChoiceTextSaved]}>
                          {cadence}
                        </Text>
                      </TouchableOpacity>
                    );
          ...
          <View style={styles.paymentNote}>
            ...
            <View style={styles.paymentCopy}>
              ...
              <Text style={styles.paymentText}>
                {isLaundry ? "Mobile Money, card, or cash options appear before you confirm." : "You can choose Mobile Money, card, cash, or pay after the quote is approved."}
              </Text>
            </View>
          </View>
        </ScrollView>
...
const styles = StyleSheet.create({ 
  ...
  photoShade: { ...StyleSheet.absoluteFillObject, backgroundColor: "rgba(28,18,8,0.33)" }, 
  photoLabel: { position: "absolute", left: 15, bottom: 14, paddingHorizontal: 9, paddingVertical: 6, borderRadius: 9, backgroundColor: "rgba(5,150,105,0.90)" }, 
  photoLabelText: { color: "#FFFFFF", fontFamily: "Inter_700Bold", fontSize: 9, letterSpacing: 0.85 }, 
  intro: { gap: 7, paddingHorizontal: 2 }, 
  label: { color: palette.green, fontFamily: "Inter_700Bold", fontSize: 9, letterSpacing: 1.2 }, 
  title: { fontSize: 26, lineHeight: 33 }, 
  summary: { fontSize: 14, lineHeight: 21, maxWidth: 330 }, 
